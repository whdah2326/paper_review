...
class Generator(nn.Module):
    ...
    def _make_block(self, in_channels:int, out_channels:int, kernel_size:int, stride:int, padding:Union[int,str]=1, mode:str='d'):
        ...
        if mode.lower() == 'd':
            ...
            block.append(nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride=stride, padding=padding, output_padding=1)) # output size를 input과 같게 해주기 위한 설정.
...
class Discriminator(nn.Module):
    def __init__(self, n_layers:int=4, input_c:int=3, n_filter:int=64, kernel_size:int=4):
        ...
        PatchGAN 구조 사용 -> 이미지를 patch로 넣어주겠다는 것 X. receptive field를 이용하기 위해서?
...
# hook function은 forward 이후에 activate된다는 점 유의.
def hook_fn(m, _, o):
    """
    m: module
    i: input
