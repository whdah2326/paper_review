...
def decay_after_k_epoch(optim: Optimizer, n_epochs:int, init_lr: float, target_lr: float, decay_after:int=100):
    ...
    scheduler2 = LinearDecayLR(optim, initial_lr = init_lr, target_lr=target_lr, total_iters=n_epochs-decay_after) # constant는 그냥 한 번 줄여주고 마는 것.
...
class LinearDecayLR(_LRScheduler):
    ...
    def __init__(self, optimizer:Optimizer, initial_lr: float, target_lr:float, total_iters:int, last_epoch:int=-1, verbose:bool=False):
        ...
        super(LinearDecayLR, self).__init__(optimizer, last_epoch, verbose) # 부모클래스의 init에 필요한 arg 전달.
        # super init을 해도 self.base_lrs 상속을 못받는 이유?
        # 여기서 부모 메소드 init 과정에서 self.get_lr을 호출. 하지만 get_lr에는 self.subtract_lr이 들어가 있고, 그건 다시 부모클래스 init이 "완료"되어야 하기 때문에 
        # 아래 명령을 실행한 적이 없다. 그래서 계속 해당 attribute이 없다고 뜨는 것.
...
class DelayedLinearDecayLR(LinearDecayLR):
    ...
    def get_lr(self):
        ...
        if self.decay_after <= self.last_epoch < (self.decay_after + self.total_iters): # 여기에 total iter도 고려.
            return [group['lr']-self.subtract_lr for group in self.optimizer.param_groups]
