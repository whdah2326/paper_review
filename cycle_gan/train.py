...
def train(opt):
    ...
    device = torch.device(f"mps:{opt.gpu_id}" if torch.backends.mps.is_available() else 'cpu')
    ...
    if opt.resume_from:
        ...
        if "LinearDecay" in getattr(type(scheduler_D), '__name__'): # when using LinearDecayLR, not loading state dict for optimizers and schedulers.
            ...
            # decay after 같은 부분 수정은 config 자체에서 받아오는 걸로 / last epoch만 수정
            scheduler_dict = {
    ...
    for epoch in range(start_epoch, start_epoch+opt.n_epochs):
        ...
        for step, data in pbar:
            ...
            optim_G.step() # alternating training 위해 G랑 D는 optimizer 따로 쓰는 것.
...
def main():
    ...
    # wandb logging init
    wandb.init(project=opt.prj_name, name=opt.exp_name, entity="j_yoon", config=vars(opt)) # save a configuration of experiments on an wandb session.
